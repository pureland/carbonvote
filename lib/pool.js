            ...
            function (balance, cb) {
                ...
                if (self.type == self.againVote)
                    self.redis.multi([
                        ...
                        console.log("err:",err)
                        console.log('decrease balance from vote ', amountKeyOld, ':', balance, "add balance to new vote", amountKeyNew, ":", currentBalance)
                        cb(null)
                        })
        ...
        async.waterfall([
            ...
            function(account,op, cb) {
                ...
                self.redis.hget(key, account, function(err, res) {
                    if (res) {
                        let keyVote=self.option_prefix + res
            ...
            function(account, prevBalance, key,keyVote,cb) {
                ...
                self.redis.multi([
                    ['incrbyfloat', self.VoteAmountTable,keyVote, balance-prevBalance],
                    ['hset', key, account, balance]
                ]).exec(function(err, res) {
                    console.log('update account', self.VoteAmountTable, ':',key, account)
